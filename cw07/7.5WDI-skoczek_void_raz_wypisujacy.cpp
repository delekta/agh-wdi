//bool znaleziony, by zakonczyc gdy znajdziemy jedno rozwiazanie!!!
#include <iostream>
using namespace std;
const int rozm = 5;
...
//bool znaleziony przeazywany przez referencje
void skoczek_szachowy(int t[rozm][rozm], int rozm, int w, int k, bool &znaleziony, int licznik = 1){
    ...
    if(licznik == rozm * rozm){
        ...
        for(wariant = 1; wariant <= 8 && (!znaleziony); wariant++)
            if(czy_mozna(t, rozm, w, k, newW, newK, wariant))
                skoczek_szachowy(t, rozm, newW, newK, znaleziony, licznik + 1);
    }
    t[w][k] = 0;
}
int main()
{
    ...
    skoczek_szachowy(tab, rozm, y, x, znaleziony);
    return 0;
}
